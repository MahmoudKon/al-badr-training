    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group mb-3">
                <label class="form-label required">
                    @lang('clients.name')
                    <a href="{{ routeHelper('clients.create') }}" class="btn btn-sm btn-primary open-modal float-end"> <i class="fas fa-plus px-0"></i> </a>
                </label>
        ...
        <div class="col-md-6">
            <div class="form-group mb-3">
                <label class="form-label required">
                    @lang('stores.name')
                    <a href="{{ routeHelper('stores.create') }}" class="btn btn-sm btn-primary open-modal float-end"> <i class="fas fa-plus px-0"></i> </a>
                </label>
        ...
        <div class="col-md-4">
            <div class="form-group mb-3">
                ...
                <div class="input-icon mb-3">
                    <input type="date" value="{{ old('bill_date', date('Y-m-d')) }}" name="bill_date" class="form-control">
    ...
    <script>
        $(function() {
            ...
            $('body').on('keyup focus', '#item_name', function(e) {
                if ([9, 16, 17, 20, undefined].includes(e.keyCode) || (e.keyCode >= 30 && e.keyCode <= 40)) {
                    return ;
                }
                ...
                ajax_request = $.ajax({
                    ...
                    type: "GET",
                    data: {store_id: $('#stores').val(), name: $(this).val()},
                    success: function(response) {
                        $('#items').empty().removeClass('d-none');
                        $.each(response, function(key, val) {
                            $('#items').append(`<li class="selected-item" data-name="${val}" data-id="${key}">${val}</li>`);
                        });
                    }
                });
            });
            ...
            function checkItemCount(id)
            {
                if ($(`tr#item-row-id-${id}`).length)  {
                    let count = Number( $(`tr#item-row-id-${id}`).find(".item-qty").text() ) + Number( $('#quantity').val() );
                    $(`tr#item-row-id-${id}`).find(".item-qty").text( count );
                    reCalcItem();
                } else {

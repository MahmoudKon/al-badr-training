@forelse ($rows as $row)
    <tr>
        ...
        <td>
            <a href="{{ route('dashboard.roles.edit', $row) }}" class="btn btn-sm btn-primary open-modal">
                Edit
                <i class="fas fa-edit"></i>
            </a>
        </td>
        <td>
            <form action="{{ route('dashboard.roles.destroy', $row) }}" method="post" class="submit-form">
                @csrf
                @method('DELETE')
